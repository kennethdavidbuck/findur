...
- AR14 — Drive connection/account inventory refresh from the leased minute worker every 24 hours and repair incomplete inventory without requiring browser activity; returning login and page rendering do not synchronously call SnapTrade. Persist and serve normalized snapshots without refetching; collapse concurrent work onto guarded operations; bound provider calls, rate limits, retries, and circuit breaking; honor `Retry-After` and provider reset headers; never trigger billable manual refresh automatically; and distinguish disabled brokerage connection repair from expired OAuth reauthorization.
