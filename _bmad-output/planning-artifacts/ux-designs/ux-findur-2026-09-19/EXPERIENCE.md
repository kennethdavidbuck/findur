...
| `Account Inclusion Control` | Post-OAuth and Portfolio settings | A labelled checkbox group framed as choosing accounts for the user's persistent dating profile. Individual usable accounts are checkboxes; Select all exposes checked/mixed/unchecked. Closed accounts are omitted. A temporarily unavailable account appears only when its state is actionable or it was previously saved. Selected and saved coverage remain distinct in ordinary language. One compact, friendly review names additions/removals, the resulting count, and that the choice can be changed later; it does not expose provider, purge, data-category, derivation, or cache internals. Saving, success, and failure are announced once without moving focus. |
...
After OAuth, `Account Inclusion Control` is a labelled checkbox group with **none selected by default**. Each usable account has a checkbox. Closed accounts are omitted from the chooser and its counts. A temporarily unavailable account remains visible only when the user needs an actionable explanation, especially when it was previously saved. **Select all** reports checked, mixed, or unchecked across usable accounts. Discovery requires **at least one** committed included account. Selected coverage is labelled separately from saved coverage in ordinary profile language.

Edits remain a draft until one compact review and Save. The review names accounts being added or removed, shows the resulting selected count, and reminds the user that the choice can be changed later. It contains no provider-access essay, data-category inventory, purge warning, private-purpose list, or implementation detail. Additions begin retrieval/derivation only after confirmation. The saved selection persists across ordinary login, refresh, staleness, temporary unavailability, and reauthorization; newly discovered accounts begin unselected. Only an explicit edit or complete disconnect/deletion removes a saved choice. Inclusion remains editable from the Portfolio Showcase.

The initial journey is `OAuth success → Choose accounts → Portfolio Showcase`. A successful committed first save navigates to the Showcase; a pending or failed save remains on Choose accounts with concise feedback. The Showcase provides **Edit included accounts**, which opens the same control with saved choices preselected and returns to the Showcase after a successful edit.

The user-facing onboarding progress is `Connect → Choose accounts → Portfolio showcase → Preferences & privacy → Preview & discover`. Portfolio retrieval and private signal building are automatic work between user actions, not forms or technical steps. The Portfolio Showcase appears immediately after Choose accounts and provides the path to continue profile setup. A ready user later lands in Discovery; an incomplete user resumes the first unmet prerequisite. Desktop onboarding replaces the ordinary app rail with this progress context; the compact mobile local strip uses `Connect → Choose accounts → Portfolio`, and later account editing uses the ordinary app shell rather than replaying onboarding.
...
3. OAuth returns success and focuses the result heading. The friendly **Choose what Findur may use.** step appears with no accounts selected; closed accounts do not appear.
4. She makes a draft selection, reviews the changed account names and resulting count in a compact modal, and saves. Only then does source retrieval/derivation begin; success is announced without technical lifecycle language.
5. The successful save opens Portfolio immediately. The Showcase shows accounts, balances/values, holdings/positions, activities/orders, coverage, and Freshness Indicator with distinct empty/unavailable/unsupported states.
...
2. Account Inclusion Control presents one labelled checkbox group. Closed accounts are omitted; a temporarily unavailable saved account keeps a concise actionable explanation. Select all reports unchecked/mixed/checked.
3. She selects two accounts. “2 selected” remains distinct from the previously saved count without exposing committed/draft implementation terminology.
4. A compact review names accounts added or removed, shows the resulting count, and says the choice can be changed anytime. Maya saves; only then does retrieval/derivation begin.
5. A concise status announces success and navigates to the Portfolio Showcase. Discovery unlocks only when at least one included account forms a Usable Portfolio and the remaining profile prerequisites are complete.
6. Later, **Edit included accounts** from the Showcase opens the same selector with saved choices preselected. Saving returns to the Showcase; ordinary later login and reauthorization do not ask her to choose again.
...
Failure: zero selected keeps Discovery gated. A failed addition retains the prior narrower committed set. The user sees concise profile-language recovery; backend purge/recalculation behavior remains governed by the permission-change matrix and is not narrated as an alarming implementation inventory in this ordinary profile edit.
