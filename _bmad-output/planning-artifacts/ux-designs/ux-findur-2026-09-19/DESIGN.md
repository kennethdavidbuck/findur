...
| `Account Inclusion Control` | One compact list of account rows with provider/account label, selection control, and only actionable availability context. Closed accounts are omitted. Selected rows use secondary outline plus checked state; “Select all” is a normal labelled control. Selected and saved counts use ordinary profile language. A compact review shows only changed account names, resulting count, “change anytime” reassurance, and Back/Save actions—never destructive styling or technical consequence prose. The screen reads as dating-profile setup while retaining Constellation geometry and trust. |
