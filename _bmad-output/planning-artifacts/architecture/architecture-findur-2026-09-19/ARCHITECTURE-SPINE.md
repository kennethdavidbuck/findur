...
- **Rule:** A timeout or transport outcome that could have reached the token endpoint is ambiguous because rotation may already have consumed the refresh token. Do not replay it automatically. If provider success cannot be committed, or the lease expires without a safely classifiable pre-send failure, transition to reauthorization-required with a guarded short transaction.
- **Rule:** Refresh shortly before expiry. On API `401`, refresh once and retry the safe request once. A second failure clears usable credentials and requires authorization again.
...
- **Rule:** Retain an older normalized version only while a current-card/preview anchor references it or during a short bounded cleanup window, then delete it. Do not build indefinite financial history. Generated users use the same versioned snapshot structure as OAuth users.
- **Rule:** Disconnect or removal from Account Inclusion immediately suppresses affected views/use, invalidates anchors/signals, and deletes whole affected account-scoped versions plus derived, cached, prefetched, and restoration state. Never delete rows from an otherwise retained immutable version. Only categorical audit/security records without financial values may remain.
...
### AD-8 — Refresh is activity-driven, bounded, and stale-while-revalidate [ADOPTED]
...
- **Rule:** Authenticated activity may initiate a freshness check for that same user's included accounts. Heartbeats and synthetic-candidate views never directly call SnapTrade. Serve the last trustworthy, freshness-labelled snapshot while bounded refresh runs.
- **Rule:** Respect connection freshness mode, account sync status, documented `X-RateLimit-Reset` and `X-RateLimit-Account-Reset` headers, and `Retry-After` when supplied. Poll supported real-time endpoints no more frequently than SnapTrade's recommended ten-minute interval per active account. Never trigger billable manual refresh automatically.
- **Rule:** Keep rate limiting separate from circuit breaking. Use `golang.org/x/time/rate` and `github.com/sony/gobreaker/v2`. Count transport failures, timeouts, and `5xx`; do not trip for `401`, `429`, or ordinary `4xx`. Retry only safe reads with a strict bound and jitter.
