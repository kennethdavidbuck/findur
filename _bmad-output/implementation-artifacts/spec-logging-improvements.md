...
|----------|--------------|---------------------------|----------------|
| Anonymous request | Public, unmatched, or protected request without a valid session | Completion log retains only safe request fields and omits `user_id` | Existing categorical HTTP response remains unchanged |
| Authenticated read | Protected GET successfully returns an Actor | Completion log and any handler event using that request context include the Actor UUID as `user_id` | Repository/session failure does not emit an identity |
| Authorized mutation | Valid session and all CSRF/origin/fetch defenses pass | Completion event includes `user_id` after authorization and otherwise retains safe route/status/latency fields | CSRF/origin failures omit identity because authorization did not succeed |
...
- `backend/internal/platform/httpapi/httpapi.go` -- owns request-ID creation, request context, request-completion event, safe route categories, and tests that prohibit raw unmatched paths. Add narrowly scoped request log metadata, set `user_id` once, and read its snapshot at every contextual log emission and when the request completes.
...
**Acceptance Criteria:**
- Given an authenticated owner makes a protected request, when session authentication succeeds, then its completion log contains that server-derived UUID as `user_id`, `request_id`, route category, method, status, and latency, without raw request inputs.
- Given an HTTP handler emits an `slog` event with the request context after authentication, when it is written, then it includes the same `request_id` and `user_id` as the completion event.
- Given an anonymous request or an invalid session/CSRF/origin check, when it is logged, then it has no `user_id` and the API’s existing response semantics are unchanged.
...
| SnapTrade account IDs can be overwritten in request metadata | false | The current handler has one authorized inclusion writer; the one-time requirement applies to `user_id`, which is guarded against replacement. |
...
| Failed unsafe authorization did not prove identity omission | low | Fixed: a real valid session with missing CSRF asserts the resulting request log omits `user_id`. |
