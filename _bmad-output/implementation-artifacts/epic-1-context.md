...
- Account Inclusion uses a named checkbox group, visible unavailable reasons, an accessible tri-state Select All over usable rows, and separate draft versus committed coverage. Confirmation names additions/removals and consequences; Cancel is neutral and restores focus.
...
Story 1.1 establishes the committed inclusion set and initial snapshots consumed by the Showcase, refresh, and trace stories. Story 1.2 establishes the normalized dataset and freshness presentation used by Stories 1.3 and 1.4. Story 1.3 maintains trustworthy versions without weakening inclusion. Story 1.5 supersedes every other operation: its lifecycle fence and deletion boundary must defeat concurrent inclusion, refresh, normalization, derivation, and publication work. Epic 1 depends on Epic 0's authenticated session, masked account inventory, and proven provider adapter; Epic 2 depends on Epic 1's committed snapshots and freshness model.
