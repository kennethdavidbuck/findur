...
function Navigation({ route, navigating = false, onNavigate }: Pick<Props, 'route' | 'navigating' | 'onNavigate'>) {
  ...
  return (
    <nav className="authenticated-nav authenticated-nav--adaptive" aria-label={messages.authenticated.navigation} aria-busy={navigating || undefined}>
      {destinations.map((destination) => (
        <a key={destination.route} href={destination.route} aria-current={route === destination.route ? 'page' : undefined} aria-disabled={navigating || undefined} onClick={(event) => handleClick(event, destination.route)}>
...
export function AuthenticatedLayout({ children, route, setup = false, navigating = false, loggingOut, logoutFailed, onNavigate, onLogout }: Props) {
  const { messages } = useI18n()
  return (
    <div className={`authenticated-shell${setup ? ' authenticated-shell--setup' : ''}`}>
      ...
      {!setup && <Navigation route={route} navigating={navigating} onNavigate={onNavigate} />}
      {navigating && <p className="visually-hidden" role="status">{messages.authenticated.checking}</p>}
